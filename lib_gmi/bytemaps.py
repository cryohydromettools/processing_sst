...
class Dataset:
    ...
    def _get_variables(self):
        ...
        for var in self._variables(): data[var] = self._get_avariable(var,bmap)
        return data
...
class Verify:
    ...
    For files organized as arrays:
        startline = starting line number of data for each variable (dictionary)
    ...
    def _get_avariable(self,var):
        ...
        for ilat,ilon in indices: data.append(self._get_oneob(var,ilon,ilat))
        return data

    def _get_data(self):
        data = OrderedDict()
        for var in self.variables: data[var] = self._get_avariable(var)
        return data
